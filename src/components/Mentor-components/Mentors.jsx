...
const CARD_WIDTH = 273 + 16; 

function MentorCard({ mentor }) {
    const theme = colorThemes[mentor.color];
    return (
        <div
            className={`flex-shrink-0 w-[273px] bg-white rounded-2xl border-t-4 ${theme.border} shadow-sm overflow-hidden flex justify-center items-center flex-col `}
            style={{ minHeight: 300 }}
        >
...
export default function TopMentors() {
  ...
  useEffect(() => {
    const calculateMaxIndex = () => {
      ...
      const newMax = mentors.length - visibleCards;
